...
Category = {'LINEAR';'DWELL';'PSOCONTROL';'CW&CCW';'INCREMENTAL&ABSOLUTE';'PT';'Other'};
...
while ~feof(f)
    ...
    switch s{1}{1}
        ...
        case 'PSOCONTROL',count.Count(3)=count.Count(3)+1;
            switch s{1}{3}
                case {'ON','ARM'},lineColor = [0 0 0];lineWidth = 1.5;
        ...
        case 'PT',time.Time(4)=time.Time(4)+LINEAR(s{1}(2:end)');count.Count(6)=count.Count(6)+1;
        case 'INCREMENTAL',isABSOLUTE=0;count.Count(5)=count.Count(5)+1;
        case 'ABSOLUTE',isABSOLUTE=1;count.Count(5)=count.Count(5)+1;
        case 'G92',G92(s{1}{2:end});count.Count(end)=count.Count(end)+1;
        case 'PROGRAM',if size(s{1},1)==3&&strcmp(s{1}{3},'STOP'),PgmStopCheck = 0;end,count.Count(end)=count.Count(end)+1;
        case {'G359','ENABLE','METRIC','SECONDS','VELOCITY','PSOOUTPUT',...
                'DVAR','FILECLOSE','COMMINIT','COMMSETTIMEOUT','ARRAY',...
                'FILEWRITE','CRITICAL','PSOTRACK','PSODISTANCE','END'},count.Count(end)=count.Count(end)+1;
        otherwise
